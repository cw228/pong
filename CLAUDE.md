...
- **`src/main.cpp`** — constructs `Window`, `InputState`, and `Renderer`, runs the main loop with delta time calculation; loop order: `glfwPollEvents()` → `updateInputState()` → `updateGameState()` → `drawFrame()`
- **`src/inputstate.h`** / **`src/inputstate.cpp`** — `InputState` struct (mouse position + key states); `updateInputState()` polls GLFW each frame
...
- **`src/gamestate.h`** / **`src/gamestate.cpp`** — `GameState`, `Entity`, `Instance`, `Level`, `Model`, `Texture` structs; `loadGameState(path)` parses `Pong.json` (generated by `pong.py`); `updateGameState(gameState, inputState, deltaTime)` runs game logic each frame
...
**`SV_InstanceID` in Slang/SPIR-V:** Slang's `SV_InstanceID` compiles to `gl_InstanceIndex - gl_BaseInstance`, which is always 0-based (does **not** include `firstInstance`). To get the equivalent of `gl_InstanceIndex` (which includes the `firstInstance` offset), use `SV_StartInstanceLocation + SV_InstanceID`. This is needed when `firstInstance` is used to index into an SSBO of model matrices.
...
**GPU hang / system freeze:** If running the app freezes the system (black screen, blinking cursor), the GPU is hanging. Common causes:
- Shader accesses a descriptor (UBO, texture) that was never bound. Always ensure descriptor sets are created, updated, and bound before draw calls that use them.
- Per-frame CPU→GPU buffer `memcpy` writes past the end of allocated GPU memory. In `updateRenderState`, `renderState.instances` **must be cleared at the start of the function** before repopulating from `GameState`. Without the clear, the vector grows unboundedly every frame; once it exceeds `MAX_INSTANCES`, the `memcpy` in `updateStorageBuffer` overflows the mapped storage buffer and causes a GPU fault.
